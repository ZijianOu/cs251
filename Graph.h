...
class graph {
        ...
        for(edge &e : vertices[u].outgoing) {
          ...
          if(report[v].state == UNDISCOVERED) {
            ...
            q.push(v);
			  ...
			  else if (report[v].dist > report[u].dist) {
				  report[v].dist = report[u].dist+1;
				  report[v].pred = u;
				  report[v].npaths = report[u].npaths;
			  }	
  ...
  public:
     ...
     *
     */
    bool extract_path(const vector<vertex_label> & rpt, //<----------------------------------------------------BUG!!
	  ...
	  while (rpt[u].pred != u && rpt[u].pred != -1) {
		  tempvec.push_back(rpt[u].pred);
		  u = rpt[u].pred;
		  //extract_path(rpt, dest, path);
	  }
	  ...
	  while (!q.empty()) {
		  ...
		  for (edge &e : vertices[u].outgoing) {
			  ...
			  if (rpt[v].state == UNDISCOVERED) {
				  ...
				  else if (rpt[v].dist == rpt[u].dist + e.weight) {
					  rpt[v].preds.push_back(u);
				  }
    ...
    bool dag_num_paths(vector<vertex_label> & rpt) {
	 ...
	 for (int i = 0; i < rpt.size(); i++) {
	  ...
	  rpt.clear();
	  vector<int>topo;
	  if (topo_sort(topo) != true) {
		  return false;
	  }
	  for (int i = 0; i < vertices.size(); i++) {
		  rpt.push_back(vertex_label(0, -1, UNDISCOVERED, 0));
	  }
	  /*for (int i = 0; i < vertices.size(); i++)
	  {
	  ...
	  for (int i = 0; i < topo.size(); i++) {
		  for (edge &e : vertices[topo[i]].outgoing) {

			  rpt[e.vertex_id].npaths = rpt[e.vertex_id].npaths + rpt[topo[i]].npaths;
		  }
	  }
	  for (int i = vertices.size() - 1; i >= 0; i--) {
		  for (edge &e : vertices[topo[i]].incoming) {
			  rpt[e.vertex_id].dist = rpt[e.vertex_id].dist + rpt[topo[i]].dist;
		  }
	  }
	  for (int i = 0; i < rpt.size(); i++) {
		  rpt[i].npaths = rpt[i].npaths * rpt[i].dist;
	  }
    ...
    bool enum_paths(int target, vector<string> &paths) {
	  ...
	  for (int i = 0; i < vertices.size(); i++) {
		  if (vertices[i].incoming.size() == 0) {
			  path.push_back(i);
			  //topo_sort(path);
			  enum_paths_help(target, path, paths);
			  path.clear();
		  }
	  }
      // your code here!
	...
	void enum_paths_help(int target, vector<int>&path, vector<string>&paths) {
		int v;
		string str;
		for (edge& e : vertices[path.back()].outgoing) {
			...
			if (path.back() == target) {
				for (int i = 0; i < path.size() - 1; i++) {
					str = str + id2name(path[i]) + " ";
				}
				str = str + id2name(path.back());
				paths.push_back(str);
				//return;
			}
			/*for (path.back() == target) {
				enum_paths_help(target, path, paths);
			}*/
			enum_paths_help(target, path, paths);
			path.pop_back();
...
//	//----------------------------------------------------------------------
//	// returns:
//	//   1 if x is an ancestor of y
//	//   2 if y is an ancestor of x
//	//   3 if x and y have no ancestor/descendant relationship
//	// precondition:  assumes both x and y are in tree and x!=y
//	// NOTE:  your function operates at the "node level".  No enclosing
//	//          object to worry about.
//	//        (t is a pointer to the root of the bst under consideration)
//	bool checkAncestor(bst_node *t, int x, int y) {
//		if (t == nullptr) {
//			return false;
//		}
//		if (t->val == x) {
...
//		}
//		if (x < t->val) {
//			return checkAncestor(t->left, x, y);
//		}
//		if (x > t->val) {
//			return checkAncestor(t->right, x, y);
//		}
//		return false;
//	}
//
//	//-------------------------------------------------------------------------------------
//	static int relation(bst_node *t, int x, int y) {
//		if (checkAncestor(t, x, y)) {
//			return 1;
//		}
//		else if (checkAncestor(t, y, x)) {
//			return 2;
//		}
//		else {
//			return 3;
...
//	struct node {
//		double price;      // price of this option
//		double time;       // time of this option
//		double min_time;   // minimum/fastest time among all options in subtree
//						   //    rooted at this node            
//		node *left;
//		node *right;
//	};
...
//	// For your convenience:  function _min_time takes a node pointer t and
//	//          returns the minimum/fastest time among all nodes in subtree rooted
//	//          at t:
//	//       If t==nullptr INFINITY is returned (since t is an empty subtree)
//	//       otherwise, returns t->min_time
//	static double _min_time(node *t) {
//		if (t == nullptr)
//			return INFINITY;
//		else
//			return t->min_time;
//	}
//	// TODO:  complete fastest
//// description:   returns minimum achievable time for the given budget
////    among the options in tree rooted at t.  If all options in tree are priced higher
////    than given budget, INFINITY is returned.
////
//// RUNTIME:  O(h) where h is the height of the tree
////
//// NOTE:  your function operates at the "node level".  No enclosing
////        object to worry about.
//	static double fastest(node *t, double budget) {
//		if (t == nullptr) {
//			return false;
//		}
//		//while (t != nullptr) {
...
//		if (t->price < budget) {
//			//curr = t;
//			if (t->right->left->price < budget) {
//				t = t->right;
//			}
//			if (t->right->left->price > budget) {
//				if (t->time != t->min_time) {
//					t - t->left;
//				}
//				else {
//					return t->time;
...
//			}
//		}
//		if (t->left == nullptr&&t->price > budget) {
//			return INFINITY;
//		}
//
//	}
...
//
//
//	void helper(int target, string str, vector<string>& paths, int spath, int path) {
...
//				//paths.push_back(e.vertex_id);
//				//if (paths.back() == target) {
//				//	for (int i = 0; i < paths.size() - 1; i++) {
//				//		str = str + id2name(path[i]) + " ";
//				//	}
//				//	str = str + id2name(paths.back());
//				//	paths.push_back(str);
//				//	//return;
//				//}
//			helper(e.vertex_id, id2name(e.vertex_id) + " " + str, paths, spath, path + e.weight);
//		}
//	}
//		void enum_spaths(std::vector<vertex_label> &rpt, int u,
//			std::vector<string> &spaths) {
//			string str = " ";
//			if (has_cycle() || u < 0 || u >= num_nodes() || rpt[u].dist == -1) {
//				return;
//			}
//			str = str + id2name(u);
//			helper(u, str, spaths, rpt[u].dist, 0);
//
//		}
	//	
	//
